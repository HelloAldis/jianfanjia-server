...
exports.user_wenjuan = function (req, res) {
  ...
  url = url.replace(/{appid}/g, config.wechat_appid).replace(
    /{redirect_uri}/g, redirect_uri).replace(/{state}/g, state);
  res.redirect(url);
}

exports.wechat_index = function (req, res) {
  var redirect_uri = 'http%3a%2f%2f{host}';
  var url =
    'https://open.weixin.qq.com/connect/oauth2/authorize?appid={appid}&redirect_uri={redirect_uri}&response_type=code&scope=snsapi_userinfo&state={state}#wechat_redirect';
  var state = utility.randomString(16, 'abcdef0123456789');
  redirect_uri = redirect_uri.replace(/{host}/g, req.headers.host);
  url = url.replace(/{appid}/g, config.wechat_appid).replace(/{redirect_uri}/g, redirect_uri).replace(/{state}/g, state);
