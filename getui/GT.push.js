  ...
  if (instance[appkey] == null) {
    ...
    if (serviceMap[this._appkey] == null) {
      this.getOSPushDomainUrlList(function (err, resp) {
        if (resp != null && resp["result"] == "ok" && resp["osList"].length >
          0) {
 ...
 *           push result
 */
GeTui.prototype.pushMessageToSingle = function (message, target, requestId,
  callback) {
...
GeTui.prototype.getContentId = function (message, taskGroupName, callback) {
  ...
  this.httpPostJson(this._host, postData, false, function (err, response) {
    if (!err && response.result === 'ok' && response.contentId) {
      ...
      callback && callback(new Error('host:[' + this._host + ']' +
        '获取contentId失败'), response);
    }
  });
...
GeTui.prototype.cancelContentId = function (contentId, callback) {
  ...
  this.httpPostJson(this._host, postData, false, function (err, response) {
    if (!err && 'ok' === response.result) {
      ...
      callback && callback(new Error('host:[' + host + ']' +
        '取消contentId失败'), false);
    }
  });
...
GeTui.prototype.pushMessageToList = function (contentId, targetList, callback) {
  ...
  if (targetList.length > limit) {
    callback && callback(new Error('target size:' + targetList.length +
      ' beyond the limit:' + limit), null);
    return;
  }
 ...
 * @param callback
 */
GeTui.prototype.pushAPNMessageToSingle = function (appId, deviceToken, message,
  callback) {
...
GeTui.prototype.pushAPNMessageToList = function (appId, contentId,
  deviceTokenList, callback) {
